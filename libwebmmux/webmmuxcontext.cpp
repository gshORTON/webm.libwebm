...
void Context::Open(std::basic_iostream<unsigned char>* pStream)
{
    ...
    if (pStream)
    {
        ...
        const long long One_GB = 1024i64 * 1024i64 * 1024i64;
        long long size = 8 * One_GB;
...
unsigned long Context::GetTimecodeScale() const
{
   return m_timecode_scale;
}


unsigned long Context::GetTimecode() const
{
...
void Context::WriteEbmlHeader()
{
    ...
    const long long size_ = stop_pos - start_pos;
...
void Context::FinalSegment()
{
    ...
    const long long maxpos = m_file.GetPosition();
    m_file.SetSize(maxpos);

    const long long size = maxpos - m_segment_pos - 12;
    assert(size >= 0);
...
void Context::InitFirstSeekHead()
{
    ...
    const unsigned char size = /* 4 */ 3 * 21;
...
{
    const long long start_pos = m_file.SetPosition(m_first_seekhead_pos + 4 + 1);
...
void Context::WriteSeekEntry(unsigned long id, long long pos_)
{
    //The SeekID is 2 + 1 + 4 = 7 bytes.
    //The SeekPos is 2 + 1 + 8 = 11 bytes.
    //Total payload for a seek entry is 7 + 11 = 18 bytes.
...
#ifdef _DEBUG
    const long long start_pos = m_file.GetPosition();
#endif
    ...
    const long long pos = pos_ - m_segment_pos - 12;
    assert(pos >= 0);
...
void Context::InitInfo()
{
    ...
    const long long newpos = m_file.GetPosition();

    const long long size_ = newpos - pos;
...
void Context::WriteTrack()
{
    ...
    const long long end_pos = m_file.GetPosition();
...
void Context::Cluster::Final(
    ...
    const Cluster& next)
{
    const long long size_ = next.m_pos - m_pos;
    assert(size_ >= 8);
    assert(size_ <= ULONG_MAX);
    ...
    const Keyframe& k)
{
...
#ifdef _DEBUG
    const long long start_pos = f.GetPosition();
#endif
...
#ifdef _DEBUG
    const long long start_track_pos = f.GetPosition();
#endif
    ...
    const long long off = c.m_pos - m_segment_pos - 12;
    assert(off >= 0);
...
#ifdef _DEBUG
    const long long stop_pos = f.GetPosition();
...
void Context::WriteCues()
{
    ...
    const unsigned long size = static_cast<unsigned long>(size_);
    ...
    StreamVideo::VideoFrame* pFrame)
{
    ...
    const unsigned long vt = pFrame->GetTimecode();
    ...
    else
    {
        ...
        const unsigned long vt0 = pvf0->GetTimecode();
        assert(vt >= vt0);

        const double dt = double(vt) - double(vt0);
        assert(dt >= 0);

        const double scale = GetTimecodeScale();
        assert(scale >= 1);

        const double ns = scale * dt;
    ...
    const unsigned long at = paf->GetTimecode();
    ...
    StreamAudio::AudioFrame* pFrame)
{
    ...
    const unsigned long at = pFrame->GetTimecode();
    ...
    if ((m_pVideo == 0) || (m_pVideo->GetFrames().empty() && m_bEOSVideo))
    {
        ...
        const unsigned long at0 = paf->GetTimecode();
        assert(at >= at0);
    ...
    if (m_bEOSVideo)
    {
        ...
        const unsigned long vt = static_cast<unsigned long>(vt_);
    ...
    const unsigned long vt = pvf->GetTimecode();  //2nd rframe
    assert(vt >= vt0);
...
bool Context::WaitVideo() const
{
    ...
    const unsigned long vt0 = pvf0->GetTimecode();
    const unsigned long vt = pvf->GetTimecode();
...
bool Context::WaitAudio() const
{
    ...
    const unsigned long vt0 = pvf->GetTimecode();
    ...
    const StreamVideo::VideoFrame* pvf_stop)
{
    ...
    if (pPrevCluster)
    {
        const long long size = c.m_pos - pPrevCluster->m_pos;
        assert(size > 0);
    ...
    while (!vframes.empty())
    {
        ...
        const unsigned long vt_stop = pvf_stop->GetTimecode();
        ...
        const unsigned long at_stop = paf_stop->GetTimecode();
    ...
    const long long pos = m_file.GetPosition();

    const long long size_ = pos - c.m_pos - 8;
    assert(size_ <= ULONG_MAX);
...
void Context::CreateNewClusterAudioOnly()
{
    ...
    const unsigned long af_first_time = af_first.GetTimecode();
    ...
    unsigned long cFrames = 0;   //TODO: must write cues for audio
    ...
    while (!aframes.empty())
    {
        ...
        const unsigned long t = paf->GetTimecode();
...
void Context::WriteVideoFrame(Cluster& c, unsigned long& cFrames)
{
    assert(m_pVideo);
    ...
    const unsigned long ft = pf->GetTimecode();
...
void Context::WriteAudioFrame(Cluster& c, unsigned long& cFrames)
{
   assert(m_pAudio);
